...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const enrollment of enrollments as Enrollment[]) {
      try {
        // 2a. Fetch campaign — skip if not active
        ...
        // 2f. Variable substitution in template params
        ...
        // 2j. Advance to next step
        const nextStep = (steps as Step[]).find(
