...
interface EmailCampaignListProps {
  campaigns: EmailCampaignWithStats[];
  total: number;
  page: number;
  perPage: number;
  totalPages: number;
...
export function EmailCampaignList({
  ...
  async function handleDelete() {
    if (!deleteId) return;
    const result = await safeFetch(`/api/campaigns/email?id=${deleteId}`, {
      method: "DELETE",
    });
    setDeleteId(null);
    if (!result.ok) {
  ...
  async function handleTogglePause(campaign: EmailCampaignWithStats) {
    const newStatus = campaign.status === "active" ? "paused" : "active";
    const result = await safeFetch(`/api/campaigns/email?id=${campaign.id}`, {
      method: "PATCH",
        ...
        <EmptyState
          ...
          description={
            hasFilters
              ...
              : "Create your first email campaign to reach your prospects."
          }
          action={
            !hasFilters
              ? {
